...
각 검색어 × 활성 채널을 검색합니다. 기본 6 × 8 = 48개 검색 조합 중 하루 최대 32회(재시도 포함)를 순환 실행, 검색당 최대 20개 결과이며 중복 URL을 통합합니다. 검색량이 설정 한도를 넘으면 다음 실행에서 남은 조합부터 순환합니다. API 요금은 사용 중인 플랜을 확인하세요.
...
- 하루 API 호출 상한은 재시도를 포함해 32회이며, UTC 달력 기준 월 상한은 1,000회입니다. 31일에도 정규 호출은 최대 992회입니다. 수동 실행도 저장된 동일 카운터를 사용합니다. 남은 검색 조합은 다음 날 이어서 처리합니다.
- 이 카운터는 이 저장소의 수집기 사용량만 관리합니다. 같은 키의 다른 앱 사용량, 수집 기록 삭제, 강제 중단이나 결과 커밋 실패는 반영할 수 없으므로 Brave 대시보드에서도 사용량을 확인하세요.

- 매일 한국 시간 09:17 예약(UTC 00:17). GitHub 예약 실행은 지연될 수 있습니다.
- 공개 저장소는 60일간 저장소 활동이 없으면 예약이 비활성화될 수 있습니다. 데이터 커밋이 보통 활동을 만들지만 자동화 상태를 정기적으로 확인하세요.
- 일부 API 요청 실패 시 성공 결과를 반영하고 실패 상태를 표시합니다. 전체 실패 시 기존 결과를 유지합니다. 마지막 성공이 30시간 전이면 화면에 경고합니다.
